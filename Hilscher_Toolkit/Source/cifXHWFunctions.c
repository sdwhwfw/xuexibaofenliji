 ...
 **************************************************************************************
   ...
   Filename:
    $Id: cifXHWFunctions.c 2871 2011-09-14 13:44:06Z Robert $
   Last Modification:
    $Author: Robert $
    $Date: 2011-09-14 15:44:06 +0200 (Mi, 14 Sep 2011) $
    $Revision: 2871 $
   ...
   Changes:
     ...
     ----------------------------------------------------------------------------------
     36        13.09.2011  RM       Added DEV_DoSystemBootstart()
     35        29.03.2011  MT       Lock DEV_DoSystemStart againt DSR to prevent writes to
                                    handshake flags while in reset, if an IRQ occurs
     34        02.03.2011  SS       Macros HWIF_READ8/16/32 now routed to read wrapper functions
     33        01.03.2011  MT       - DEV_DoSystemStart now clears internal COS Flags in Interrupt mode
     32        31.01.2011  SS       Hardware interface layer added to support 
                                    serial DPM access (e.g. via SPI)
     31        01.06.2010  MT       - DEV_CheckCOSFlags is now locked against DEV_DoSystemStart
                                      to prevent concurrent access to handshake flags
     30        23.04.2010  MT       DEV_ReadHandshakeFlags changed to provide locking
                                    of systemstart with the internal handshake bit state
     29        22.04.2010  MT       - Removed Writing of Slot number to device. This was
                                      necessary for firmwares that did not support slot numbers.
                                    - Readout of Device Handshake cells added on system start
     28        21.04.2010  MT       - DMA Triple Buffer structure updated
     27        29.03.2010  MT       Added function DEV_GetIOBitstate()
     26        22.02.2010  MT        - DEV_ReadHandshakeFlags modified to only check COS flags if
                                       the channel layouts have already been read.
                                     - DEV_WaitForBitState_Irq may spin multiple times on low timeout values
     25        03.12.2009  RM       DEV_GetMBXState, RecvCount handling fixed

     24        06.11.2009  MT       Workaround: If firmware does not provide slot number, but
                                                bootloader does, the toolkit overwrites the
                                                Slot Number with the value from bootloader
     23        01.10.2009  MT       - Host COS flag handling moved to internal function
                                      DEV_DoHostCOSChange
                                    - DEV_WaitForBitstate_IRQ changed, so it works correctly for
                                      COS handshake if Interrupt does an extra COS Handshake

     22        30.09.2009  RM       - Handling of the COS xx_ENABLE flags changed.
                                      xxx_ENABLE flags are directly removed if the
                                      Enable state was signaled to the hardware
                                    - DEV_ReadHostFlags extended by parameter fReadHostCOS,
                                      to prevent reading back the own COS flags (only during SYSTEM_START)

     21        25.09.2009  RM       DEV_GetMBXState, RecvCount handling improved

     20        07.07.2009  RM       DEV_BusState()
                                    - Inserted to be callable as a low level function
                                    - Handling of COM_FLAG in conjunction with timeout settings
                                      fixed/improved

     19        18.05.2009  MT       Bugfix:
                                    - Big Endian Problem in DEV_CheckCOSFlags, which could result in
                                      Communication COS flags being handled incorrectly.
     18        18.03.2009  MT       - DEV_WaitForBitState_Irq was not able to check for clear/set flags,
                                      which is needed for BusState and ConfigLock functions.
     17        17.03.2009  MT       - DEV_ReadHostFlags did not read Application COS

     16        13.02.2009  MT       - DEV_CheckCOSFlags is now locked agains DEV_DoChannelInit,
                                      to prevent COS Flag evaluation while a channel init is running

     15        20.01.2009  MT       Added:
                                     - Big endian host support added (set #define CIFX_TOOLKIT_BIGENDIAN)
                                     - DEV_WaitForReady_Poll() internal sleep() before checking the DPM
                                       reduced to 10ms

     14        17.06.2008  RM       - Included OS_Sleep(0) in functions where a
                                      while loop is used (DEV_WaitForReady_Poll(),
                                      DEV_WaitForBitState_Poll()).
                                    - OS_Sleep() called after checking for timeout

     13        03.04.2008  RM       - DEV_DoChannelInit() does not wait any more until
                                      card is RUNNING again. READY is sufficient.
                                      If ulTimeout = 0, no check READY is done
                                    - DEV_TriggerWatchdog() now checks ptChannel != NULL

     12        18.03.2008  RM       - DEV_GetHostState() error return changed from
                                      CIFX_DEV_NOT_RUNNING to CIFX_DEV_NOT_READY

     11        26.04.2007  RM       - Unneccessary DEV_ReadHandshake() removed
                                    - Checking of interrupt mode before calling
                                      DEV_ReadHandshake() included (except in poll functions)

     10        19.04.2007  RM       - DoSystemstart now checks for invalid DPM entries and
                                      takes this as not READY

      9        17.04.2007  RM       - Parameters of DEV_TransferPacket() extended
                                      and function now will do a callback if
                                      unexpected packets are received

      8        13.04.2007  RM       - DEV_DoSystemStart()/DEV_DoChannelInit()
                                      using default timeouts CIFX_TO_WAIT_HW
                                      if ulTimeout parameter is 0

      7        10.04.2007  RM       - Changed name of WARMSTART and COLDSTART to
                                      SYSTEM_START and CHANNEL_INIT

      8        04.04.2007  RM       - New Functions DEV_WaitForReady/NotRead and
                                      DEV_WaitForRunning/NotRunning() included
                                    - Coldstart / Warmstart reworked

      7        02.04.2007  RM       Changed COS flags to new definition
                                    - DEV_SetHostState()
                                    - DEV_DoWarmstart()/DEV_DoColdstart()
      6        27.03.2007  RM       - COLDSTART/WARMSTART reset COS flags and signal new state
      5        21.03.2007  MT       - Functions calling DEV_ReadHandshake Flag now check fIrqEnabled flag
      ...
      4        16.03.2007  MT       DEV_IsCommunicating did non set CIFX_DEV_NO_COMFLAG correctly
      3        13.10.2006  MT       DEV_WaitForBitState_Poll's timeout changed.
                                     - Starttime was read inside poll loop
                                     - Starttime read into uint32_t, for overflow detection
                                       a int32_t variable is needed
                                    DEV_ReadHandshakeFlags() wrong HCF_NETX_COS_ACK flag fixed
      2        06.10.2006  MT       Put/GetPacket now checks for Ready flag
...
{
  uint16_t 	usCOSCmdBitMask = 0;
  uint16_t 	usCOSAckBitMask = 0;
  uint32_t* pulCOSFlags     = NULL;
  ...
  if(ptChannel->fIsSysDevice)
  {
    ...
    usCOSCmdBitMask = NSF_NETX_COS_CMD;
    usCOSAckBitMask = HSF_NETX_COS_ACK;

    pulCOSFlags = (uint32_t*)&ptSysChannel->tSystemState.ulSystemCOS;
  } else
  {
    /* This is a communication channel */
    usCOSCmdBitMask = NCF_NETX_COS_CMD;
    usCOSAckBitMask = HCF_NETX_COS_ACK;

    if(NULL != ptChannel->ptCommonStatusBlock)
      pulCOSFlags = (uint32_t*)&ptChannel->ptCommonStatusBlock->ulCommunicationCOS;
  }
  ...
  if(NULL != pulCOSFlags)
  {
    if((ptChannel->usNetxFlags ^ ptChannel->usHostFlags) & usCOSCmdBitMask)
    {
      uint32_t ulNewCOSFlags = LE32_TO_HOST(HWIF_READ32(ptDevInstance, *pulCOSFlags)); /* Read actual COS flags */
  ...
  switch (ulCmd)
  {
    case CIFX_BUS_STATE_ON:
    {
      ...
      if(ulTimeout && (CIFX_DEV_NO_COM_FLAG == lRet))
      {
        ...
        if (DEV_WaitForBitState( ptChannel, NCF_COMMUNICATING_BIT_NO, RCX_FLAG_SET, ulTimeout))
        {
          lRet = CIFX_NO_ERROR;
        } else
        {
...
int32_t DEV_SetHostState(PCHANNELINSTANCE ptChannel, uint32_t ulNewState, uint32_t ulTimeout)
{
  ...
  ulTimeout = ulTimeout;    /* prevent compiler warnings */
...
int32_t DEV_DMAState(PCHANNELINSTANCE ptChannel, uint32_t ulCmd, uint32_t* pulState)
{
  ...
  *pulState = (ptChannel->ptCommonStatusBlock->ulCommunicationCOS & RCX_COMM_COS_DMA) ?
...
{
  uint8_t bData = 0;
  ptDev->pfnHwIfRead(ptDev, (uint32_t)pvSrc, &bData, 1);
  return bData;
}
...
{
  uint16_t usData = 0;
  ptDev->pfnHwIfRead(ptDev, (uint32_t)pvSrc, &usData, 2);
  return usData;
}
...
{
  uint32_t ulData = 0;
  ptDev->pfnHwIfRead(ptDev, (uint32_t)pvSrc, &ulData, 4);
  return ulData;
}
