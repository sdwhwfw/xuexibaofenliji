 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile: cifXDownload.c $
   Last Modification:
    $Author: MichaelT $
    $Modtime: 31.08.09 11:01 $
    $Revision: 1583 $
   ...
   Changes:
     ...
     ----------------------------------------------------------------------------------
     23       09.06.2010   MT       Larger loadable modules (.NXO) failed to download, as the last packet
                                    starts relocation on the target and the confirmation may take longer
                                    as the internal timeout.
     22       11.05.2010   RM       DEV_DownloadFile(), uses twice the timeout for the last packet if
                                    modules are loaded because relocation of modules are done with the 
                                    receiving the last packet 
     21       07.05.2010   RM       DEV_DeleteFile() OS_Strncpy() size of copied data must include
                                    the necessary 0 character 
     20       29.04.2010   MT       Get MD5 Request may not copy complete filename into request,
                                    depending on implementation of OS_Strncpy
     19       21.04.2010   MT       Fixed GCC compiler warning (deferencing pointer breaks strict-aliasing rules)
     18       05.08.2009   MT       Bugfix:
                                     - DEV_Download returned CIFX_BUFFER_TOO_SHORT,
                                       if another packet arrived in the mailbox, instead of calling
                                       pfnRecvCallback, due to internal buffer size limitation
     17       28.04.2009   RM       Added:
                                     - Function DEV_ProcessFWDownload()

     16       20.01.2009   MT       Added:
                                     - Big endian host support added (set #define CIFX_TOOLKIT_BIGENDIAN)

     15       16.01.2009   RM       DEV_CheckForDownload() changed, because rcX still calculates
                                    the MD5 over the whole file. So it must be also calculated

     14       18.11.2008   RM       - HilFileHeaderV3 included
                                    - DEV_CheckForDownload() now checks the file header version.
                                      If < V3 than MD5 is calculated otherwise just compared

     13       02.05.2007   RM       - Comment fixed  (line 96)

     12       23.04.2007   RM       - Checking of valid firmware file and
                                      Delete file included

     11       19.04.2007   RM       - Checking of valid file extensions for
                                      firmware files included

     10       17.04.2007   RM       - Parameters for pfnTransferPacket() extended
                                    - Parameters for DEV_UploadFile()/DownloadFile() extended

     9        13.04.2007   RM       - Upload/Download, send abort detection and application
                                      progress,changed

     8        12.04.2007   RM       - Number of bytes read from file fixed

     7        05.04.2007   RM       - Size calculation of maximum block size in
                                      Download/Upload fixed

     6        03.04.2007   RM       - Download/Upload finished fixed, checking now for
                                      packet extension LAST and NONE

     5        30.03.2007   RM       - Using now RCX_Public.h instead of MidSys_Public.h
                                    - Send packets completly initalized
                                    - MID_SYS definition changed to RCX_

     4        29.03.2007   RM       Timeout values for transfer packet taken from cifXUser.h

     3        28.03.2007   RM       DEV_DownloadFile/UploadFile changed to be useable in
                                    cifx32dll project.
                                    - Packet transfer function passed as function pointer
                                    - Parameter definitions changed

     2        20.03.2007   MT       Added:
                                    - DEV_Upload
     1        11.07.2006   MT       initial version
...
{
  int32_t         lRet          = CIFX_NO_ERROR;
  PDEVICEINSTANCE ptDevInstance = ((PCHANNELINSTANCE)pvChannel)->pvDeviceInstance;
  ...
  } else
  {
    ...
    } else
    {
      ...
      if(g_ulTraceLevel & TRACE_LEVEL_INFO)
      {
        USER_Trace(ptDevInstance,
                  TRACE_LEVEL_INFO,
                  "MD5 not identical, process firmware download");
      }
    }
  }
      ...
      } else
      {
        ...
        if( DEV_IsNXFFile(pszFileName))
        {
          ...
          } else
          {
            ...
            {
              DEV_RemoveChannelFiles(hSysDevice, ulChannel, pfnTransferPacket, NULL, NULL, NULL);
            }
        ...
        } else if( DEV_IsNXOFile(pszFileName))
        {
          ...
          } else
          {
            ...
            /* We have to delete existing files, depending of a NXF/NXO */
            /* Leave NXF file */
            DEV_RemoveChannelFiles( hSysDevice, ulChannel, pfnTransferPacket, NULL, NULL, HIL_FILE_EXTENSION_FIRMWARE);
