 ...
 **************************************************************************************
   ...
   Filename:
    $Id: cifXHWFunctions.h 3302 2012-02-07 10:28:52Z Robert $
   Last Modification:
    $Author: Robert $
    $Date: 2012-02-07 11:28:52 +0100 (Di, 07 Feb 2012) $
    $Revision: 3302 $
   ...
   Changes:
     ...
     ----------------------------------------------------------------------------------
     5         07.02.2012  RM       - Removed ulCOMState from NETX_COM_STATE_T
     4         31.01.2012  SD       - Added NETX_COM_STATE_T -> CHANNELINSTANCEtag (COM-State notification)
     3         13.09.2011  RM       - Added eCIFX_DEVICE_DONT_TOUCH
                                    - Added DEV_DoSystemBootstart()
     2         02.03.2011  SS       Macros HWIF_READ8/16/32 now routed to read wrapper functions  
     1         28.02.2011  SS       Declarations for cifX hardware functions separated 
                                    from cifXToolkit.h
...
typedef struct CIFX_SYNCH_DATA_Ttag
{
  ...
  #define HWIF_READN(ptDev, Dst, Src, Len) ((PDEVICEINSTANCE)ptDev)->pfnHwIfRead(ptDev, (uint32_t)Src, Dst, Len)
  #define HWIF_WRITE8(ptDev, Dst,  Src)                       \
  do {                                                        \
    uint8_t bData = Src;                                      \
    ((PDEVICEINSTANCE)ptDev)->pfnHwIfWrite(ptDev, (uint32_t)&(Dst), (void*)&bData, 1);  \
  } while (0);
  #define HWIF_WRITE16(ptDev, Dst, Src)                       \
  do {                                                        \
    uint16_t uiData = Src;                                    \
    ((PDEVICEINSTANCE)ptDev)->pfnHwIfWrite(ptDev, (uint32_t)&(Dst), (void*)&uiData, 2); \
  } while (0);
  #define HWIF_WRITE32(ptDev, Dst, Src)                       \
  do {                                                        \
    uint32_t ulData = Src;                                    \
    ((PDEVICEINSTANCE)ptDev)->pfnHwIfWrite(ptDev, (uint32_t)&(Dst), (void*)&ulData, 4); \
  } while (0);
  #define HWIF_WRITEN(ptDev, Dst, Src, Len) ((PDEVICEINSTANCE)ptDev)->pfnHwIfWrite(ptDev, (uint32_t)Dst, Src, Len)
