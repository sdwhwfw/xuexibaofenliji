 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile: cifXFunctions.c $
   Last Modification:
    $Author: sebastiand $
    $Modtime: 9.11.09 8:34 $
    $Revision: 3277 $
   ...
   Changes:
     ...
     Version   Date        Author   Description
      ----------------------------------------------------------------------------------
     37        31.01.2012  SD       - Added COM-State notification (CIFX_NOTIFY_COM_STATE)
     36        14.12.2011  RM       - Added xSysdeviceExtendedMemory()
     35        18.04.2011  SS       Optional validation of pointers and handles passed 
                                    to toolkit functions (CIFX_TOOLKIT_PARAMETER_CHECK)
     34        31.01.2011  SS       Hardware interface layer added to support 
                                    serial DPM access (e.g. via SPI)
     33        12.08.2010  RM       xChannelDMAState() comments fixed
     32        30.04.2010  MT       xChannelPLCActivateRead/Write did not lock access to handshake flags
     31        28.04.2010  MT       xChannelPLCMemoryPointer may not work correctly due to
                                    an invalid parameter passed to OS_MapUserPointer / UnmapUserPointer
     30        21.04.2010  MT       Fixed GCC compiler warning (deferencing pointer breaks strict-aliasing rules)
     29        15.04.2010  MT       - Added timeout to xChannelSyncState function
                                    - Added CIFX_SYNC_WAIT_CMD to be able to wait for sync signalling by firmware
     28        29.03.2010  MT       - IOMode / HandshakeBitState is now read from common status block if available
                                      Fallback is to use the modes read at startup during channel enumeration
     27        05.03.2010  RM       - Sync function and event handling added
      
     26        02.11.2009  RM       - Definition CIFX_TOOLKIT_DMA to enable DMA function
                                      included
                                    - Definition CIFX_TOOLKIT_NO_ERRORLOOKUP included to
                                      prevent error text table creation included

     25        30.10.2009  RM       - xChannelDMAState() added
                                    - DMA handling in IORead and IOWrite added

     24        01.10.2009  MT       - Host COS flag handling moved to internal function
                                      DEV_DoHostCOSChange

     23        07.07.2009  RM       xChannelBusState()
     ...
     21        18.05.2009  MT       Bugfix:
                                     - Big Endian problems in
                                        ...
                                        * xChannelConfigLock (CIFX_CONFIGURATION_GETLOCKSTATE could result in invalid
                                          locking state)
     20        28.04.2009  RM       Added:
                                     - New download type module included and xSysdeviceDownload()/
                                       xChanneldDownload() changed / extended
                                     - Error lookup table extended

     19        17.03.2009  MT       Bugfix:
                                     - xChannelBusState and xChannelConfigLock did not work on Big Endian machines

     18        20.01.2009  MT       Added:
                                     - Big endian host support added (set #define CIFX_TOOLKIT_BIGENDIAN)

     17        31.07.2008  RM       - C++ comments changed to C comments

     16        17.06.2008  RM       - OS_MapUserPointer() and OS_UnmapUserPointer() now
                                      have a new parameter pvOSDependent

     15        16.05.2008  RM       - xChannelOpen() now returns either CIFX_INVALID_BOARD
                                      or CIFX_INVALID_CHANNEL
     14        06.05.2008  MT       - xDriverErrorDescripton did not return error description,
                                      due to internal array size miscalculation
     13        28.04.2008  MT       - PLC Functions did not return NO_COMFLAG error.
     12        21.02.2008  RM       - Error return for xDriverMemoryPointer() and
                                      xChannelPLCMemoryPtr() fixed

     11        24.04.2007  RM       - DEV_BusState() now checks for COM-Flag if timeout
                                      is given

     10        19.04.2007  RM       - Download chooses now the transfer type
      ...
      8        17.04.2007  RM       - xSysdeviceDownload(),
      ...
      6        02.04.2007  RM       Changed COS flags to new definition
                                    - DEV_SetHostState()
                                    - DEV_DoWarmstart()/DEV_DoColdstart()

      5        28.03.2007  RM       - xDriverEnumChannels() returns CIFX_NO_MORE_ENTRIES
                                      instead of CIFX_INVALID_CHANNEL.
                                    - xChannelPLCMemory(), check DEV_IsRunning() removed
                                    - xChannelPLCMemory(), check DEV_IsRunning() removed

      4        27.03.2007  RM       Mutex added into I/O and Packet functions, to
                                    prevent multiple access
      3        20.03.2007  MT       Added:
      ...
      2        09.01.2007  RM       Missing function declarations for
...
// for debug added by bruce.zhu 2013.08.09
...
static const CIFX_ENDIANESS_ENTRY_T s_atCommonStatusBlock[] =
{
  ...
  { 0x00, eCIFX_ENDIANESS_WIDTH_32BIT, 3}, /* ulCommCos,ulCommState,
                                              ulCommError                    */
  { 0x0C, eCIFX_ENDIANESS_WIDTH_16BIT, 4}, /* usVersion,usWatchDogTime
                                              ausReserved[2]                 */
  { 0x14, eCIFX_ENDIANESS_WIDTH_32BIT, 5}, /* ulHostWatchDog, ulErrorCount,
                                              ulErrorLogInd,  aulReserved[2] */
  { 0x20, eCIFX_ENDIANESS_WIDTH_32BIT, 6}, /* ulSlaveState, ulSlaveErrLogInd
  ...
  switch(ulCmd)
  {
    ...
    case CIFX_INFO_CMD_SYSTEM_CHANNEL_BLOCK:
      ...
      } else
      {
        ...
        for(iChannel = 0;
            iChannel < sizeof(ptSysChannel->atChannelInfo) / sizeof(ptSysChannel->atChannelInfo[0]);
            ++iChannel)
        {
...
int32_t APIENTRY xDriverGetErrorDescription(int32_t lError, char* szBuffer, uint32_t ulBufferLen)
{
  ...
  for(iIdx = 0; iIdx < (uint32_t)(sizeof(s_atErrorToDescrTable) / sizeof(s_atErrorToDescrTable[0])); ++iIdx)
  {
  ...
  switch (ulNotification)
  {
    case CIFX_NOTIFY_RX_MBX_FULL:
      ...
      } else
      {
        ...
        if(DEV_WaitForBitState(hChannel, 
    ...
    case CIFX_NOTIFY_SYNC:
      ...
      } else
      {
        ...
        if(DEV_WaitForSyncState(hChannel, 
                                bState,
                                0))
        {
          pfnCallback(ulNotification, 0, NULL, pvUser);
  ...
  {  
    switch (ulCmd)
    {
      case CIFX_SYNC_SIGNAL_CMD:
        ...
        } else if(!DEV_WaitForSyncState(hChannel, RCX_FLAGS_EQUAL, ulTimeout))
        {
        ...
        {
          /* Signal new sync */
          DEV_ToggleSyncBit( ptChannel->pvDeviceInstance, (1 << ptChannel->ulChannelNumber));
      ...
      case CIFX_SYNC_ACKNOWLEDGE_CMD:
        ...
        } else if(!DEV_WaitForSyncState(hChannel, RCX_FLAGS_NOT_EQUAL, ulTimeout))
        {
          /* Sync cannot be signalled as bits are in wrong state */
          lRet = CIFX_DEV_SYNC_STATE_TIMEOUT;
        } else
      ...
      case CIFX_SYNC_WAIT_CMD:
        {
          ...
          } else
          {
            ...
            /* Wait for sync */
            if(!DEV_WaitForSyncState(hChannel, bState, ulTimeout))
            {
              /* Sync timeout */
              lRet = CIFX_DEV_SYNC_STATE_TIMEOUT;
            } else
