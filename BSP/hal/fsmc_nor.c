...
/* Delay definition */
#define BlockErase_Timeout         ((uint32_t)0x01000000)
#define ChipErase_Timeout          ((uint32_t)0x60000000) 
#define Program_Timeout            ((uint32_t)0x00002400)

#define ADDR_SHIFT(A)              (NOR_FLASH_START_ADDR + (2 * (A)))
#define NOR_WRITE(Address, Data)   (*(vu16 *)(Address) = (Data))

#define NOR_BUFFER_SIZE            0x1000
#define NOR_WRITE_READ_ADDR        0x0

/* Private variables ---------------------------------------------------------*/
static uint16_t TxBuffer[NOR_BUFFER_SIZE];
static uint16_t RxBuffer[NOR_BUFFER_SIZE];
...
void FSMC_NOR_Test(void)
{
...
#if 1
	usart_send_string(COM1, "Erase Whole chip...");
	state = FSMC_NOR_EraseChip();
	if (state == NOR_SUCCESS)
	{
	...
	while(1)
	{
		...
		FSMC_NOR_ReturnToReadMode();
		sprintf((char*)vl_buf, "[%d]Nor Flash ID = %04X,%04X,%04X,%04X	", run_times++, 
		...
		{
			usart_send_string(COM1, "Type = AM29LV128ML\r\n");
		}
		else if ( ( NOR_ID.Manufacturer_Code == 0x0001 ) && ( NOR_ID.Device_Code1 == 0x227E ) &&
		( NOR_ID.Device_Code2 == 0x2221 ) && ( NOR_ID.Device_Code3 == 0x2201 ) )
		{
			usart_send_string(COM1, "Type = S29GL128P\r\n");
		}
		else if ( ( NOR_ID.Manufacturer_Code == 0x0020 ) && ( NOR_ID.Device_Code1 == 0x227E ) &&
		( NOR_ID.Device_Code2 == 0x2220 ) && ( NOR_ID.Device_Code3 == 0x2200 ) )
		{
			usart_send_string(COM1, "Type = M29DW128F\r\n");
		}
		else
		{
			usart_send_string(COM1, "Type = Unknow\r\n");
		}

		/* Erase the NOR memory block to write on */
		state = FSMC_NOR_EraseBlock(NOR_WRITE_READ_ADDR + pos);
		if (state == NOR_SUCCESS)
		{
			//usart_send_string(COM1, "Erase Block OK!\r\n");
		}
		else
		{
			usart_send_string(COM1, "Erase Block Error!\r\n");
		}
		...
		for (index = 0; index < NOR_BUFFER_SIZE; index++ )
		{
			TxBuffer[index] = index + 0xaaaa;
		}

		state = FSMC_NOR_WriteBuffer(TxBuffer, NOR_WRITE_READ_ADDR + pos, NOR_BUFFER_SIZE);
		if (state == NOR_SUCCESS)
		{
			//usart_send_string(COM1, "NOR_WriteBuffer OK!\r\n");
		}
		else
		{
			usart_send_string(COM1, "NOR_WriteBuffer Error!\r\n");
		}

		/* Read data from FSMC NOR memory */
		FSMC_NOR_ReadBuffer(RxBuffer, NOR_WRITE_READ_ADDR + pos, NOR_BUFFER_SIZE);

		if( memcmp( (char*)TxBuffer, (char*)RxBuffer, NOR_BUFFER_SIZE ) == 0 )
		{
			//usart_send_string(COM1, "Nor Flash is OK \r\n");
		}
		else
		{
			for(index = 0; index < NOR_BUFFER_SIZE; index++)
			{
				...
				{
					snprintf((char*)vl_buf, sizeof(vl_buf), "[%d]rx[%4x]-tx[%4x]\r\n", index, RxBuffer[index], TxBuffer[index]);
					usart_send_string(COM1, vl_buf);
				}
			}
